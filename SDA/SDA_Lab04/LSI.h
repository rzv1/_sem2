...
private:
    ...
    void grow(int new_dim) {
		...
		for (int i = old_dim; i + 1 < new_dim; i++) 
			elems[i].next = i + 1;
    ...
    int alocare(int valoare) {
        if (primulLiber == -1) {
            grow(elems.empty() ? 8 : (int)elems.size() * 2);
        }
        int poz = primulLiber;
...
public:
    ...
    bool stergeValoare(int valoare) {
        ...
        int target = (prev == -1) ? -1 : elems[prev].next;
    ...
    const NodeArr& at(int poz) const {
		return elems[poz];
    }
};
